...
function Invoke-TitleCardCreation {
    ...
    try {
        if (-not $episode.RootFoldername) {
            Write-Entry -Subtext "Skipping episode - missing RootFoldername: $($episode.'Show Name')" -Path $global:configLogging -Color Yellow -log Warning
            return
        }
        ...
        if (-not $global:episode_numbers -or $global:episode_numbers.Count -eq 0) {
            Write-Entry -Subtext "Skipping season $($episode.'Season Number') of $($episode.'Show Name') - no episodes" -Path $global:configLogging -Color Yellow -log Warning
            return
        }
    ...
    Else {
        ...
        Write-Entry -Subtext "Error in Invoke-TitleCardCreation for $($episode.'Show Name'): $($_.Exception.Message)" -Path $global:configLogging -Color Red -log Error
        $global:errorCount = Increment-GlobalStat 'errorCount'
    }
}
