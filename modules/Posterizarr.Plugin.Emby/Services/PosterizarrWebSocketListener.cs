...
namespace Posterizarr.Plugin.Services
{
    ...
    /// <summary>
    /// Background entry point service that connects to Posterizarr's real-time WebSocket API.
    /// Automatically applies asset changes to Emby items without waiting for scheduled tasks.
    /// </summary>
    public class PosterizarrWebSocketListener : IServerEntryPoint
    {
        ...
        public PosterizarrWebSocketListener(ILibraryManager libraryManager, ILogManager logManager)
        {
        ...
        private void LogDebug(string message, params object[] args)
        {
            ...
            {
                _logger.Info("[Posterizarr WS DEBUG] " + string.Format(message, args));
            }
        }
        ...
        private async Task ConnectionLoopAsync(CancellationToken ct)
        {
            ...
            while (!ct.IsCancellationRequested)
            {
                ...
                if (wsUri == null)
                {
                    _logger.Warn("[Posterizarr WS] Invalid PosterizarrApiUrl: '{0}'. Retrying in 60s.", config.PosterizarrApiUrl);
                    try
                    {
                        await Task.Delay(TimeSpan.FromSeconds(60), ct).ConfigureAwait(false);
                    }
                ...
                try
                {
                    ...
                    _logger.Info("[Posterizarr WS] Connected successfully to Posterizarr real-time event stream at {0}", wsUri);
                    retryDelaySeconds = 5; // Reset backoff on successful connect
                    _hasLoggedMissingEndpoint = false;
                    _hasLoggedConnectionRefused = false;
                ...
                catch (WebSocketException ex)
                {
                    ...
                    if (is404)
                    {
                        ...
                        {
                            _logger.Warn(
                                "[Posterizarr WS] Posterizarr server at '{0}' is running, but does not support real-time WebSocket events (HTTP 404). " +
                    ...
                    {
                        _logger.Warn("[Posterizarr WS] WebSocket connection error: {0}. Reconnecting in {1}s...", ex.Message, retryDelaySeconds);
                        retryDelaySeconds = Math.Min(retryDelaySeconds * 2, 60);
                    }
                }
                catch (Exception ex)
                {
                    _logger.ErrorException("[Posterizarr WS] Unexpected error in WebSocket listener. Reconnecting in {0}s...", ex, retryDelaySeconds);
                    retryDelaySeconds = Math.Min(retryDelaySeconds * 2, 60);
                }
        ...
        private Uri? BuildWebSocketUri(string rawUrl)
        {
            try
            {
                ...
                builder.Query = string.Empty; // STRICT SECURITY: No credentials in query string
                return builder.Uri;
            }
            catch (Exception ex)
            {
                _logger.ErrorException("[Posterizarr WS] Error building WebSocket URI from '{0}'", ex, rawUrl);
                return null;
            }
        }
        ...
        private async Task ReceiveLoopAsync(ClientWebSocket client, PluginConfiguration config, CancellationToken ct)
        {
            ...
            while (client.State == WebSocketState.Open && !ct.IsCancellationRequested)
            {
                ...
                do
                {
                    ...
                    {
                        _logger.Info("[Posterizarr WS] Server sent close: {0}", result.CloseStatusDescription);
                    ...
                    {
                        _logger.Warn("[Posterizarr WS Security] Received oversized message (> {0} bytes). Discarding frame.", MaxMessageSize);
                        return;
                    }
                ...
                if (result.MessageType == WebSocketMessageType.Text)
                {
                    ...
                    try
                    {
                        ...
                        {
                            HandleAssetUpdatedEvent(payload, config);
                        }
                    }
                    catch (Exception ex)
                    {
                        _logger.Warn("[Posterizarr WS] Failed to parse event JSON: {0}. Error: {1}", json, ex.Message);
                    }
                }
            }
        }
        ...
        private void HandleAssetUpdatedEvent(AssetEventPayload payload, PluginConfiguration config)
        {
            ...
            {
                _logger.Warn("[Posterizarr Security] Blocked path traversal attempt in relative path: '{0}'", rawRelative);
                return;
            }
            ...
            {
                _logger.Warn("[Posterizarr Security] Path traversal blocked! Target '{0}' escapes asset root '{1}'", fullTargetFile, normalizedRoot);
                return;
            }
            ...
            _logger.Info("[Posterizarr WS] Processing real-time update for '{0}' (Type: {1}, Folder: '{2}')",
                payload.Title ?? payload.FolderName, payload.AssetType, payload.FolderName);

            // =========================================================================
            // Match affected item in Emby Library
            // =========================================================================
            try
            {
            ...
            {
                _logger.ErrorException("[Posterizarr WS] Error applying real-time asset update for '{0}'", ex, payload.FolderName);
            }
        }
